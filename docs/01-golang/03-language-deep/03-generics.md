...
### 6.6 Go 1.26 新增：类型构造与循环检测优化（Type Construction and Cycle Detection）

> 这是 Go 1.26 对类型检查器的内部优化，对用户**无直接可观测行为变化**，但面试官可能借这个话题深入考察候选人对**编译器前端工作原理**的理解。

#### 6.6.1 背景：类型检查器的类型构造过程

Go 编译器在类型检查阶段，需要将 AST 中的类型表达式构造为内部数据结构。例如：
...
类型检查器先构造 `T`，遇到 `[]U` 需要先构造 `U`，遇到 `*int` 需要先构造 `int`（预定义类型，无需构造）。完成后依次回溯完成各类型。

#### 6.6.2 递归类型的困境

当类型自身引用自身时，构造过程会遇到**不完整类型**：

```go
type T []U
type U *T  // U 引用 T，但 T 还在构造中
```

**Go 1.25 及之前的处理**：
- 类型检查器在遇到不完整类型时直接报错（corner case）
- 某些递归类型场景需要特殊 hack 才能通过编译

**Go 1.26 的改进**：
- 类型检查器推迟对不完整类型的解构检查（deconstruction）
- 延迟到所有类型构造完成后再做完整性验证
- 这使得更多合法的递归类型定义可以通过编译

#### 6.6.3 面试话术

> "Go 1.26 对类型检查器做了内部重构，主要是放宽了递归类型构造时的完整性要求。以前某些递归类型定义会被编译器误判为错误，Go 1.26 通过延迟解构检查解决了这个问题。这个改进的实际意义是减少了 corner case，让类型系统更自洽，但对业务代码没有直接影响。如果被追问，可以补充说这也说明了 Go 泛型的实现一直在迭代——Go 1.26 进一步解除了泛型类型参数列表中禁止自引用的限制，结合这个类型检查器改进，使得复杂递归结构（如自引用树节点约束）的实现更自然。"
