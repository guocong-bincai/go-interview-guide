...
[![题目数量](https://img.shields.io/badge/题目-495-orange?style=flat-square)](./docs)
[![版本](https://img.shields.io/badge/版本-v5.27-blue?style=flat-square)](./docs)
...
> 共 **495** 道高频面试题 ｜ 12 大核心模块 ｜ 按面试优先级排序
...
| 02 | [**数据库**](docs/02-database/README.md) | **40** | ★★★★★ | P0 | MySQL 索引/MVCC/锁/双写一致性/Bloom Filter 防穿透/读写分离延迟/乐观悲观锁选型/Redis 数据结构选型/Redis 7 新特性与阻塞排查/ES 写入流程与深度分页/MongoDB 读写关注/ClickHouse 存储设计 |
...
| 日期 | 版本 | 更新内容 |
|------|------|----------|
| 2026-09-24 | v5.27 | 数据库模块新增 4 篇（补齐此前最薄弱的 ES / Redis 7 / MongoDB 一致性 / ClickHouse 存储设计方向）：Elasticsearch 写入流程与两阶段查询（segment 不可变主线：内存 buffer + translog → refresh 生成 segment 实现近实时、flush 落盘清 translog、merge/force merge 才真正释放删除空间、写入吞吐调优 refresh_interval/-1 + 副本先关后开、refresh=true 逐条写导致小 segment 爆炸的坑、请求 429/409 处理）、ES 两阶段查询与深度分页（query phase 各分片返回 TopN docID → 协调节点归并 → fetch phase 取 _source、max_result_window=10000 与 O(分片数×(from+size)) 根因、from+size/scroll/search_after 三方案对比与 Go search_after 游标封装、必须搭配唯一确定性排序键）、ES 集群脑裂与分片分配（7.x 自动 quorum 取代 minimum_master_nodes、master-eligible 需奇数且专用节点、initial_master_nodes 仅首次 bootstrap、green/yellow/red 归因与 _cluster/allocation/explain、磁盘 85%/90%/95% 三档水位导致只读、单分片 10~50GB 经验值）、ES 写入可靠性与双写一致性（wait_for_active_shards、translog durability request/async 取舍、Bulk 200 仍可能含逐条 failures、if_seq_no/if_primary_term 乐观锁、推荐 Canal/Debezium 订阅 binlog 而非业务双写）；Redis 7 新特性与阻塞卡顿排查（FUNCTION LOAD/FCALL 相对 EVAL 的复用与集群同步优势、no-writes/allow-oom 限制、Multi-Part AOF base RDB + incr 分离使重写不再全量重放、aof-timestamp-enabled、Sharded Pub/Sub 让订阅随分片水平扩展、maxmemory-clients 防慢客户端吃光内存、三大层排查法 --latency/--intrinsic-latency → SLOWLOG/INFO commandstats → INFO persistence/memory 看 latest_fork_usec 与 COW、慢命令黑名单 KEYS/FLUSHALL/HGETALL/DEL 大 key、五大阻塞元凶与对策、used_memory_rss 与碎片率判读）；MongoDB 副本集选举与读写关注（oplog 幂等重放、多数派 N/2+1 投票与 priority/hidden/延迟节点、writeConcern w:1 vs majority vs j:true 的数据回滚风险、readConcern local/majority/linearizable/snapshot 与 readPreference 三概念辨析、因果会话保证读己之写、Change Stream + fullDocument:updateLookup、CP 取舍与单文档原子更新优先于多文档 2PC 事务）；ClickHouse 存储设计（ORDER BY 即稀疏索引与 PRIMARY KEY 前缀关系、index_granularity 8192 与 granule 裁剪、高基数列应放排序键靠前、分区键选低基数时间维度且分区数受控、物化视图为写入触发增量预聚合需手工回填历史、SummingMergeTree 需 FINAL/sum 兜底、LowCardinality/TTL 冷热分层、点查仍应回 MySQL）。共新增 4 篇，模块题数 36 → 40，全库 491 → 495；并修正 Redis 基础篇两处错误（`activ expire 100` → `active-expire-effort 10`、maxmemory-policy 重复配置互相矛盾） |
